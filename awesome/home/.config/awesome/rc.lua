...
beautiful.init(os.getenv("HOME") .. "/.config/awesome/themes/bytem/theme.lua")

mwallpaper1 = home .. "/.settings/bg.png"
mwallpaper2 = home .. "/.settings/bg.jpg"
if awful.util.file_readable(mwallpaper1) then
	beautiful.wallpaper = mwallpaper1
elseif awful.util.file_readable(mwallpaper2) then
	beautiful.wallpaper = mwallpaper2
end
...
for s = 1, screen.count() do
    ...
    tags[s] = awful.tag({ 1, 2, 3, 4, 5, 6, 7, 8, 9, 0, "q", "w", "e", "r", "t", "y", "u", "i", "o", "p" }, s, layouts[1])
end
-- }}}
...
for i = 1, 10 do
    globalkeys = awful.util.table.join(globalkeys,
        ...
        awful.key({ modkey, "Control", "Mod1" }, "#" .. i + 9,
                  function ()
                      local screen = mouse.screen
                      local tag = awful.tag.gettags(screen)[i+10]
                      if tag then
        ...
        awful.key({ modkey, "Control", "Mod1", "Shift" }, "#" .. i + 9,
                  function ()
                      if client.focus then
                          local tag = awful.tag.gettags(client.focus.screen)[i+10]
                          if tag then
